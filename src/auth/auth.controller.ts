...
export class AuthController extends BaseController implements IAuthController {
  constructor(
    ...
    this.bindRoutes([
      ...
      },
      {
        ...
        method: "post",
        middlewares: [
          new AuthGuardMiddleware(this.authService, "users/logout"),
        ],
      },
      {
        path: "/refresh",
  ...
  async logout(
    ...
    try {
      ...
      if (refreshToken) {
        ...
        await this.authService.deleteAuthSession(
          Number(payload.id),
          refreshToken,
        );
      }
  ...
  async refresh(
    ...
    try {
      ...
      if (!refreshToken) {
        return next(
          new AuthError(401, "Missing refresh token", "auth:refresh"),
        );
      }
      ...
      const isValid = await this.authService.verifyStoredRefreshToken(
        user.id,
        refreshToken,
      );
      if (!isValid) {
        return next(
          new AuthError(401, "Invalid refresh token", "auth:refresh"),
        );
      }
      ...
      return next(
        new HTTPError(
          500,
          "Internal server error while verifying refresh token",
          "auth:refresh",
        ),
      );
    }
